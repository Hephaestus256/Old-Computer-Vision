...
void color_point(int x, int y, rgb c)
{
	_test_ds[TEST_DS_OUT_GFX_1]->write_pxl_safe(x, y, c);
	//show_windows();
}

void color_point_in(int x, int y, rgb c)
{
	_test_ds[TEST_DS_IN]->write_pxl_safe(x, y, c);
}
...
public:
	...
	void correct_verts()
	{
		for (circular_iterator<unsigned> i(verts.size(), 0, verts.size() - 1); i.cont(); i.iterate()) {
			...
			if (mx.report_key() != -1) {
				//show_point(circuit[mx.report_key()].alias(), RGB_PURPLE);
			} else {
...
public:
	...
	void trace(int lef_x, int rig_x, int y, tracer*& lef_trace, tracer*& rig_trace)
	{
		...
		if (connections.size()) { // if there were connections above segment
			...
			while (true) {
				if ((*i)->last_connection != NULL) { // diverge case - has a previous connection
					//tracer_unit t =     (*i)->last_connection->lef_trace->delete_lef();
					tracer_unit t = connections.front()->lef_trace->delete_lef();
					tracer_unit unit = (*i)->last_connection->rig_trace->delete_rig();

					/*
					show_point(t, RGB_BLUE);
					show_windows();
					show_point(unit, RGB_ORANGE);
					show_windows();
					*/
